...
import { maxSocketCount,fmtMoney } from "@/lib/metrics";
...
export function FiltersPanel({pending,setPending,setFilters}:{pending:Filters;setPending:(f:Filters)=>void;setFilters:(f:Filters)=>void}){
 ...
 const slider=(label:string,key:"maxBudget"|"minAvgCores"|"maxTdp"|"minSpec",min:number,max:number,step:number,display?:string)=><div style={{marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:5}}><span>{label}</span><span style={{border:"1px solid #38566d",borderRadius:4,padding:"6px 10px",background:"#071724"}}>{display??pending[key]}</span></div><input type="range" min={min} max={max} step={step} value={pending[key]} onChange={e=>setPending({...pending,[key]:Number(e.target.value)})} style={{width:"72%",accentColor:"#0969e8"}}/></div>;
 ...
 {slider("Max System Budget (USD)","maxBudget",100,60000,100,fmtMoney(pending.maxBudget))}{slider("Min Cores / CPU","minAvgCores",0,172,1)}{slider("Max TDP / Socket (W)","maxTdp",35,400,5)}{slider("Min SPECint2017","minSpec",0,3000,10)}
