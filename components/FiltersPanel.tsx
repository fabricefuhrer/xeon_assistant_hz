...
export function FiltersPanel({ pending, setPending, setFilters }: { pending: Filters; setPending: (f: Filters) => void; setFilters: (f: Filters) => void }) {
  ...
  return <Panel><div style={{ padding: "10px 14px", borderBottom: "1px solid rgba(97,145,175,.25)", display: "flex", justifyContent: "space-between" }}><b>FILTERS</b><button onClick={() => { setPending(defaultFilters); setFilters(defaultFilters); }} style={{ background: "transparent", border: 0, color: "#18a8ff" }}>Reset All</button></div><div style={{ padding: 12 }}>
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:4,marginBottom:10}}>
      {[
        ["Performance",{workload:"hpc-ai" as Workload,segment:"all" as const}],
        ["Value",{workload:"any" as Workload,segment:"Mainstream" as const}],
        ["TCE",{workload:"any" as Workload,segment:"TCE" as const}],
        ["Efficiency",{workload:"storage" as Workload,segment:"all" as const}]
      ].map(([label,preset])=><button key={label as string} onClick={()=>setPending({...pending,...(preset as Partial<Filters>)})} style={{padding:"6px 3px",fontSize:10,border:"1px solid #25445c",borderRadius:4,background:"rgba(255,255,255,.04)",color:"#d8e1e8"}}>{label as string}</button>)}
    </div>
    ...
    <Select label="System" value={pending.system} onChange={v => setPending({ ...pending, system: v as SystemModel, coreKind: "all", scalability: "any", socket: "any", ...((v === "st50-v3" || v === "sr250-v3") ? { workload: "any" as Workload } : {}) })} opts={Object.entries(systems).filter(([value]) => value !== "any").map(([value, item]) => [value, item.label])} />
    ...
    <Select label="Core Type" value={pending.coreKind} onChange={v => setPending({ ...pending, coreKind: v as Filters["coreKind"] })} opts={(pending.system === "sr630-v4" ? [["all", "All Core Types"], ["p", "P-Cores only"], ["e", "E-Cores only"]] : [["all", "All Core Types"], ["p", "P-Cores only"]]) as string[][]} />
