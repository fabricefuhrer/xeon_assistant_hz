...
import { fmtMoney, perfDollar, benchmarkSockets, specBase } from "@/lib/metrics";
...
function Row({label,cpu,color,top}:{label:string;cpu:Cpu|null;color:string;top:Cpu|null}){
 ...
 const sameConfig=top?benchmarkSockets(cpu)===benchmarkSockets(top):false; const perf=top&&sameConfig?Math.round((specBase(cpu)/Math.max(1,specBase(top))-1)*100):null;
