...
import { fmtMoney, perfDollar, perCpuSpec } from "@/lib/metrics";
...
function Row({label,cpu,color,top}:{label:string;cpu:Cpu|null;color:string;top:Cpu|null}){
 ...
 const perf=top?Math.round((perCpuSpec(cpu)/Math.max(1,perCpuSpec(top))-1)*100):0;
 return <div style={{border:`1px solid ${color}`,borderRadius:7,padding:8,background:"rgba(255,255,255,.03)",display:"grid",gridTemplateColumns:"1.1fr 1fr",gap:8}}><div><div style={{color,fontWeight:800,fontSize:12}}>{label}</div><div style={{fontSize:19,fontWeight:900,marginTop:3}}>Xeon {cpu.sku}</div><div style={{fontSize:11,color:"#9fb0bd"}}>{cpu.segment} • {cpu.tdpW}W • {fmtMoney(cpu.costUsd)}</div></div><div style={{fontSize:11,lineHeight:1.55,borderLeft:"1px solid rgba(97,145,175,.25)",paddingLeft:8}}><b>vs. Recommended</b><br/>Performance <span style={{color}}>{perf>0?"+":""}{perf}%</span><br/>Price <span style={{color}}>{price>0?"+":""}{price}%</span></div></div>
}
