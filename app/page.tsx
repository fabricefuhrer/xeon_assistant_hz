...
type Workload = "any" | "hpc-ai" | "database" | "web" | "cloud" | "storage";
type SystemModel = "any" | "st50-v3" | "sr250-v3" | "sr630-v4" | "sr650-v4" | "sr680a-v4" | "sr850-v4" | "sr860-v4";
...
const cpuCatalog: Cpu[] = [
  { sku: "G7400T", family: "Intel Pentium", codename: "Alder Lake", coreType: "P-Cores", cores: 2, maxTurboGHz: 3.1, baseGHz: 3.1, cacheMB: 6, tdpW: 35, costUsd: 95, chips: 1, specInt2017: 24, maxScalability: "1S", segment: "Extended" },
  { sku: "G7400", family: "Intel Pentium", codename: "Alder Lake", coreType: "P-Cores", cores: 2, maxTurboGHz: 3.7, baseGHz: 3.7, cacheMB: 6, tdpW: 46, costUsd: 105, chips: 1, specInt2017: 30, maxScalability: "1S", segment: "Extended" },
  { sku: "E-2414", family: "Xeon E", codename: "Raptor Lake", coreType: "P-Cores", cores: 4, maxTurboGHz: 2.6, baseGHz: 2.6, cacheMB: 12, tdpW: 55, costUsd: 220, chips: 1, specInt2017: 45, maxScalability: "1S", segment: "Extended" },
  { sku: "E-2434", family: "Xeon E", codename: "Raptor Lake", coreType: "P-Cores", cores: 4, maxTurboGHz: 3.4, baseGHz: 3.4, cacheMB: 12, tdpW: 55, costUsd: 270, chips: 1, specInt2017: 58, maxScalability: "1S", segment: "Extended" },
  { sku: "E-2436", family: "Xeon E", codename: "Raptor Lake", coreType: "P-Cores", cores: 6, maxTurboGHz: 2.9, baseGHz: 2.9, cacheMB: 18, tdpW: 65, costUsd: 330, chips: 1, specInt2017: 72, maxScalability: "1S", segment: "Extended" },
  { sku: "E-2456", family: "Xeon E", codename: "Raptor Lake", coreType: "P-Cores", cores: 6, maxTurboGHz: 3.3, baseGHz: 3.3, cacheMB: 18, tdpW: 80, costUsd: 410, chips: 1, specInt2017: 82, maxScalability: "1S", segment: "Extended" },
  { sku: "E-2478", family: "Xeon E", codename: "Raptor Lake", coreType: "P-Cores", cores: 8, maxTurboGHz: 2.8, baseGHz: 2.8, cacheMB: 24, tdpW: 80, costUsd: 500, chips: 1, specInt2017: 92, maxScalability: "1S", segment: "Extended" },
  { sku: "E-2486", family: "Xeon E", codename: "Raptor Lake", coreType: "P-Cores", cores: 6, maxTurboGHz: 3.5, baseGHz: 3.5, cacheMB: 18, tdpW: 95, costUsd: 560, chips: 1, specInt2017: 88, maxScalability: "1S", segment: "Extended" },
  { sku: "E-2488", family: "Xeon E", codename: "Raptor Lake", coreType: "P-Cores", cores: 8, maxTurboGHz: 3.2, baseGHz: 3.2, cacheMB: 24, tdpW: 95, costUsd: 620, chips: 1, specInt2017: 105, maxScalability: "1S", segment: "Extended" },
...
const systems: Record<SystemModel, { label: string; maxSockets: number; tower?: boolean; allowE: boolean; maxTdp?: number }> = {
  any: { label: "Any System", maxSockets: 4, allowE: true },
  "st50-v3": { label: "ThinkSystem ST50 V3", maxSockets: 1, tower: true, allowE: false, maxTdp: 125 },
  "sr250-v3": { label: "ThinkSystem SR250 V3", maxSockets: 1, allowE: false, maxTdp: 95 },
...
function isEntryServerCpu(cpu: Cpu) {
  return cpu.family === "Intel Pentium" || cpu.family === "Xeon E" || cpu.family === "Xeon 6300";
}

function isRackServer(system: SystemModel) {
  return ["sr630-v4", "sr650-v4", "sr680a-v4", "sr850-v4", "sr860-v4"].includes(system);
}

function workloadLabel(workload: Workload) {
  const labels: Record<Workload, string> = {
    any: "Optimize Choice",
    "hpc-ai": "HPC / AI",
    database: "Database & Analytics",
    web: "Web & Microservices",
    cloud: "Cloud Native",
...
function systemCompatible(cpu: Cpu, filters: Filters) {
  // Compatibility is intentionally strict: recommendation and tables must never fall back to an incompatible CPU.
  if (filters.system === "any") return false;

  const isEntryCpu = isEntryServerCpu(cpu);
  const isEntrySystem = filters.system === "st50-v3" || filters.system === "sr250-v3";
  const isRackSystem = isRackServer(filters.system);
  const isRackPCore = cpu.coreType === "P-Cores" && (cpu.family === "Xeon 6500" || cpu.family === "Xeon 6700");
  const isECore = cpu.coreType === "E-Cores";

  // ST50 V3 and SR250 V3 are entry-level systems only.
  if (isEntrySystem) return isEntryCpu;

  // Entry CPUs, Xeon E, Pentium and Raptor Lake/Xeon 6300 cannot appear on SR630 V4 or higher.
  if (isEntryCpu) return false;

  // Xeon 6700 E-Cores are supported only on SR630 V4.
  if (isECore) return filters.system === "sr630-v4";

  // Xeon 6500/6700 P-Cores are supported on SR630 V4 and higher SR systems.
  if (isRackPCore) return isRackSystem;

  return false;
}

function allowedByFilters(cpu: Cpu, filters: Filters) {
  // All server/CPU matrix rules live in systemCompatible().
...
function scoreCpu(cpu: Cpu, filters: Filters) {
  ...
  if (filters.workload === "hpc-ai") score += spec * 0.32 + cores * 2.4 + turbo * 55 + (cpu.segment === "Extended" || cpu.segment === "Specialised" ? 85 : 0) + (cpu.coreType === "P-Cores" ? 130 : -240);
...
function Select({ label, value, onChange, opts }: { label: string; value: string; onChange: (v: string) => void; opts: string[][] }) {
  return <div style={{ display: "grid", gridTemplateColumns: "1fr 1.65fr", alignItems: "center", gap: 10, marginBottom: 10, fontSize: 13 }}><label>{label}</label><select value={value} onChange={e => onChange(e.target.value)} style={selectStyle}>{opts.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select></div>;
}
...
function AlternativeCard({ title, cpu, color, note, top }: { title: string; cpu: Cpu | null; color: string; note: string; top: Cpu | null }) {
  if (!cpu) {
    return <div style={{ border: `1px solid ${color}`, borderRadius: 7, padding: 10, background: "rgba(255,255,255,.03)" }}><div style={{ color, fontWeight: 800, fontSize: 12, textTransform: "uppercase", marginBottom: 5 }}>{title}</div><div style={{ color: "#9fb0bd", fontSize: 12 }}>No compatible option in the selected CPU pool.</div></div>;
  }
  const perfDelta = top ? Math.round((cpu.specInt2017 / Math.max(1, top.specInt2017)) * 100) : 100;
  const priceDelta = top ? Math.round(((totalCost(cpu) - totalCost(top)) / Math.max(1, totalCost(top))) * 100) : 0;
  const powerDelta = top ? Math.round(((totalTdp(cpu) - totalTdp(top)) / Math.max(1, totalTdp(top))) * 100) : 0;
  return <div style={{ border: `1px solid ${color}`, borderRadius: 7, padding: 10, background: "rgba(255,255,255,.03)", display: "grid", gridTemplateColumns: "1.15fr 1.45fr 1.15fr", gap: 8, alignItems: "center" }}>
    <div>
      <div style={{ color, fontWeight: 800, fontSize: 11, textTransform: "uppercase", marginBottom: 5 }}>{title}</div>
      <div style={{ fontWeight: 900, fontSize: 18, lineHeight: 1.05 }}>Xeon {cpu.sku}</div>
      <div style={{ color: "#9fb0bd", fontSize: 10, marginTop: 4 }}>{note}</div>
    </div>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 6, fontSize: 11, textAlign: "center" }}>
      <div><b>{pCores(cpu) || eCores(cpu)}</b><br /><span style={{ color: "#9fb0bd" }}>{cpu.coreType}</span></div>
      <div><b>{cpu.specInt2017.toLocaleString()}</b><br /><span style={{ color: "#9fb0bd" }}>SPECint</span></div>
      <div><b>{fmtMoney(totalCost(cpu))}</b><br /><span style={{ color: "#9fb0bd" }}>Price</span></div>
    </div>
    {top && <div style={{ borderLeft: "1px solid rgba(97,145,175,.25)", paddingLeft: 8, display: "grid", gap: 3, fontSize: 10 }}>
      <div style={{ color: "#cbd5e1", fontWeight: 800 }}>vs. Recommended</div>
      <div>Perf: <b>{perfDelta}%</b></div>
      <div>Price: <b>{priceDelta > 0 ? `+${priceDelta}%` : `${priceDelta}%`}</b></div>
      <div>Power: <b>{powerDelta > 0 ? `+${powerDelta}%` : `${powerDelta}%`}</b></div>
    </div>}
  </div>;
}
...
export default function Page() {
  ...
  const bestPerf = topPerformance[0] || null;
  const mostP = [...filtered].sort((a, b) => pCores(b) - pCores(a))[0] || null;
  const mostE = [...filtered].sort((a, b) => eCores(b) - eCores(a))[0] || null;
  const lowestTdp = [...filtered].sort((a, b) => a.tdpW - b.tdpW)[0] || null;
  const highTurbo = [...filtered].sort((a, b) => b.maxTurboGHz - a.maxTurboGHz)[0] || null;
  const efficient = [...filtered].sort((a, b) => perfWatt(b) - perfWatt(a))[0] || null;
  const maxScalability = [...filtered].sort((a, b) => socketCount(b) - socketCount(a))[0] || null;
  const bestTce = [...filtered].filter(cpu => cpu.segment === "TCE").sort((a, b) => scoreCpu(b, filters) - scoreCpu(a, filters))[0] || null;
  const bestValueAlt = bestValue[0] || null;
  const lowestPowerAlt = lowestTdp;
  const selectedSystem = systems[filters.system];

  const radarCandidates = top ? [top, ...topPerformance.filter(c => c.sku !== top.sku).slice(0, 4)] : [];
  ...
  const slider = (label: string, key: "maxBudget" | "minAvgCores" | "maxTdp" | "minSpec", min: number, max: number, step: number, display?: string, help?: string) => <div style={{ marginBottom: 16 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13, marginBottom: 6 }}><span>{label}</span><span style={{ border: "1px solid #38566d", borderRadius: 4, padding: "7px 12px", minWidth: 68, textAlign: "center", background: "#071724" }}>{display ?? pending[key]}</span></div><input className="control-input" type="range" min={min} max={max} step={step} value={pending[key]} onChange={e => setPending({ ...pending, [key]: Number(e.target.value) })} style={{ width: "72%", accentColor: "#0969e8" }} />{help && <div style={{ fontSize: 11, color: "#9fb0bd", lineHeight: 1.4, marginTop: 5 }}>{help}</div>}</div>;
  ...
  return <main style={{ minHeight: "100vh", padding: 14, background: "radial-gradient(circle at 35% 10%,#0b2740 0,#06111d 35%,#020812 100%)" }}>
    ...
    <section className="dashboard" style={{ display: "grid", gridTemplateColumns: "320px 1fr 1.55fr 1.25fr", gap: 8 }}>
      <Panel><div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(97,145,175,.25)", display: "flex", justifyContent: "space-between" }}><b>FILTERS</b><button onClick={() => { setPending(defaultFilters); setFilters(defaultFilters); }} style={{ background: "transparent", border: 0, color: "#18a8ff" }}>Reset All</button></div><div style={{ padding: 14 }}>
        ...
        <Select label="System" value={pending.system} onChange={v => setPending({ ...pending, system: v as SystemModel, ...((v === "st50-v3" || v === "sr250-v3") ? { workload: "any" } : {}) })} opts={Object.entries(systems).filter(([value]) => value !== "any").map(([value, item]) => [value, item.label])} />
        ...
        <Select label="CPU Segment" value={pending.segment} onChange={v => setPending({ ...pending, segment: v as Filters["segment"] })} opts={[["all", "All Segments"], ["Extended", "Extended CPU"], ["Mainstream", "Mainstream CPU"], ["Specialised", "Specialised CPU"], ["TCE", "TCE"]]} />
      ...
      <div style={{ display: "grid", gap: 8 }}>
        <Panel><Title color="#38ff64">Recommendation</Title>{top ? <><div style={{ padding: 14 }}><div style={{ border: "1px solid #0b7039", borderRadius: 7, padding: 14, textAlign: "center", background: "rgba(4,55,37,.45)" }}><div style={{ fontSize: 28, color: "#39ff5f", fontWeight: 900 }}>Xeon {top.sku}</div><div style={{ fontSize: 13, color: "#58ff7a", marginTop: 7 }}>{top.family} • {top.codename} • {top.coreType}</div><div style={{ fontSize: 12, color: "#c9d6e0", marginTop: 7 }}>System: {selectedSystem.label} • Workload: {workloadLabel(filters.workload)}</div></div><div className="mini-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, marginTop: 10 }}><Mini label="P-Cores" value={pCores(top)} /><Mini label="E-Cores" value={eCores(top)} /><Mini label="Total Cores" value={totalCores(top)} /><Mini label="Max Turbo" value={`${top.maxTurboGHz} GHz`} /><Mini label="TDP / Socket" value={`${top.tdpW} W`} /><Mini label="System TDP" value={`${totalTdp(top)} W`} /><Mini label="CPU Price" value={fmtMoney(totalCost(top))} /><Mini label="Score" value={scoreCpu(top, filters)} /></div></div><div style={{ borderTop: "1px solid rgba(97,145,175,.25)", padding: 16 }}><h3 style={{ color: "#39ff5f", margin: "0 0 12px" }}>Why this CPU?</h3>{[`Best ranked fit for ${workloadLabel(filters.workload)}`, `Validated for ${selectedSystem.label}`, `Strong ${top.coreType} alignment`, `Balanced score using SPECint, cores, value, efficiency and turbo`].map(x => <div key={x} style={{ fontSize: 13, margin: "10px 0" }}><span style={{ color: "#46ff63", marginRight: 8 }}>✓</span>{x}</div>)}</div></> : <div style={{ padding: 18 }}><div style={{ border: "1px solid #ff5252", borderRadius: 7, padding: 16, background: "rgba(80,15,20,.35)" }}><div style={{ color: "#ff6b6b", fontWeight: 900, fontSize: 20, marginBottom: 8 }}>No Compatible CPU Found</div><div style={{ color: "#d8e1e8", fontSize: 13, lineHeight: 1.5 }}>No processor matches the selected server, workload and filter combination. Adjust system, core type, budget, segment, TDP or minimum SPECint2017.</div><div style={{ marginTop: 12, color: "#ffb4b4", fontSize: 12 }}>The tool will not fall back to an incompatible CPU.</div></div></div>}</Panel>
        <Panel><Title color="#39ff5f">Commercial Alternatives</Title><div style={{ padding: 10, display: "grid", gap: 7 }}><AlternativeCard title="Best TCE Option" cpu={bestTce} color="#39ff5f" note="Best compatible TCE CPU in the selected pool" top={top} /><AlternativeCard title="Best Value Option" cpu={bestValueAlt} color="#18a8ff" note="Highest SPECint2017 per dollar" top={top} /><AlternativeCard title="Lowest Power Option" cpu={lowestPowerAlt} color="#ff9d2e" note="Lowest TDP within compatible CPUs" top={top} /></div></Panel>
      </div>

      <div style={{ display: "grid", gap: 8 }}><Panel><Title>Key Metrics</Title><div style={{ padding: "8px 14px 0", color: "#9fb0bd", fontSize: 12, lineHeight: 1.35 }}><b style={{ color: "#d8e1e8" }}>Calculated from the selected compatible CPU pool only.</b> Current pool: <b style={{ color: "#18a8ff" }}>{filtered.length}</b> CPU(s). Changing System, Workload, Budget, Core Type, Segment, TDP or SPECint recalculates these values. These are not global Xeon metrics. Entry CPUs are locked to ST50 V3/SR250 V3, E-Cores are locked to SR630 V4, and Xeon 6500/6700 P-Cores are available only on SR630 V4 and higher SR systems.</div><div className="stat-grid" style={{ padding: 14, display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10 }}><Stat label="Best Performance" value={bestPerf ? bestPerf.specInt2017.toLocaleString() : "—"} /><Stat label="Best Value" value={bestValueAlt ? perfDollar(bestValueAlt).toFixed(3) : "—"} accent="#39ff5f" /><Stat label="Most P-Cores" value={mostP ? pCores(mostP) : "—"} /><Stat label="Most E-Cores" value={mostE ? eCores(mostE) : "—"} /><Stat label="Lowest Power" value={lowestTdp ? `${lowestTdp.tdpW}W` : "—"} /><Stat label="Highest Turbo" value={highTurbo ? `${highTurbo.maxTurboGHz} GHz` : "—"} /><Stat label="Best Efficiency" value={efficient ? perfWatt(efficient).toFixed(2) : "—"} /><Stat label="Max Scalability" value={maxScalability ? maxScalability.maxScalability : "—"} /></div></Panel>
      <Panel><Title>Performance Radar <span style={{ color: "#cbd5e1", fontWeight: 500, textTransform: "none" }}>(Normalized)</span> <span style={{ float: "right", color: "#9fb0bd", fontSize: 12, fontWeight: 500, textTransform: "none" }}>Higher is better</span></Title><div style={{ height: 250, padding: "4px 8px 8px" }}><ResponsiveContainer><RadarChart data={radarData} outerRadius="68%"><PolarGrid stroke="#31526a" /><PolarAngleAxis dataKey="metric" /><PolarRadiusAxis domain={[0, 100]} tick={false} axisLine={false} />{radarCandidates.map((c, i) => <Radar key={c.sku} name={`Xeon ${c.sku}`} dataKey={c.sku} stroke={["#39ff5f", "#18a8ff", "#d85aff", "#ffd400", "#ff3333"][i]} fill={["#39ff5f", "#18a8ff", "#d85aff", "#ffd400", "#ff3333"][i]} fillOpacity={0.05} />)}<Tooltip contentStyle={{ background: "#071724", border: "1px solid #25445c", color: "white" }} /><Legend /></RadarChart></ResponsiveContainer></div></Panel></div>
