...
function workloadFit(cpu: Cpu, workload: Workload) {
  ...
  if (workload === "tower") return isEntryServerCpu(cpu) ? 1.35 : 0.2;
...
function systemCompatible(cpu: Cpu, filters: Filters) {
  ...
  // ST50 V3 and SR250 V3 are entry/tower systems only.
  ...
  // Entry/tower CPUs are locked to ST50 V3 and SR250 V3 only.
...
function allowedByFilters(cpu: Cpu, filters: Filters) {
  ...
  // Entry/tower CPUs are locked to ST50 V3 and SR250 V3.
  ...
  if (filters.workload === "tower" && !isEntryServerCpu(cpu)) return false;
...
function scoreCpu(cpu: Cpu, filters: Filters) {
  ...
  if (filters.workload === "hpc") score += spec * 0.3 + cores * 2 + (cpu.coreType === "P-Cores" ? 120 : -200);
  if (filters.workload === "ai") score += turbo * 55 + (cpu.segment === "Extended" || cpu.segment === "Specialised" ? 85 : 0) + (cpu.coreType === "P-Cores" ? 100 : -200);
  if (filters.workload === "web" || filters.workload === "cloud") score += cores * 7 + efficiency * 125 + (cpu.coreType === "E-Cores" ? 220 : -120);
  if (filters.workload === "edge") score += cpu.tdpW <= 250 ? 160 : -90;
  if (filters.workload === "storage") score += socketCount(cpu) >= 2 ? 85 : 0;
  if (filters.workload === "tower") score += isEntryServerCpu(cpu) ? 320 : -500;
  ...
  return <main style={{ minHeight: "100vh", padding: 14, background: "radial-gradient(circle at 35% 10%,#0b2740 0,#06111d 35%,#020812 100%)" }}>
    ...
    <section className="dashboard" style={{ display: "grid", gridTemplateColumns: "320px 1fr 1.55fr 1.25fr", gap: 8 }}>
      <Panel><div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(97,145,175,.25)", display: "flex", justifyContent: "space-between" }}><b>FILTERS</b><button onClick={() => { setPending(defaultFilters); setFilters(defaultFilters); }} style={{ background: "transparent", border: 0, color: "#18a8ff" }}>Reset All</button></div><div style={{ padding: 14 }}>
        <Select label="Optimize Choice" value={pending.workload} onChange={v => setPending({ ...pending, workload: v as Workload, ...(v === "tower" ? { system: "st50-v3" } : {}) })} opts={[["any", "Optimize Choice"], ["database", "Database & Analytics"], ["hpc", "HPC"], ["ai", "AI"], ["web", "Web & Microservices"], ["cloud", "Cloud Native"], ["edge", "Edge"], ["storage", "Infrastructure & Storage"], ["tower", "Tower"]]} />
        <Select label="System" value={pending.system} onChange={v => setPending({ ...pending, system: v as SystemModel, ...((v === "st50-v3" || v === "sr250-v3") ? { workload: "tower" } : {}) })} opts={Object.entries(systems).map(([value, item]) => [value, item.label])} />
      ...
      <div style={{ display: "grid", gap: 8 }}><Panel><Title>Key Metrics</Title><div style={{ padding: "8px 14px 0", color: "#9fb0bd", fontSize: 12, lineHeight: 1.35 }}>These metrics summarize the filtered CPU pool: performance uses SPECint2017, value is SPECint2017 per dollar, efficiency is SPECint2017 per watt, and scalability shows the highest supported socket topology.</div><div className="stat-grid" style={{ padding: 14, display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10 }}><Stat label="Best Performance" value={bestPerf.specInt2017.toLocaleString()} /><Stat label="Best Value" value={perfDollar(bestValue[0] || top).toFixed(3)} accent="#39ff5f" /><Stat label="Most P-Cores" value={pCores(mostP)} /><Stat label="Most E-Cores" value={eCores(mostE)} /><Stat label="Lowest Power" value={`${lowestTdp.tdpW}W`} /><Stat label="Highest Turbo" value={`${highTurbo.maxTurboGHz} GHz`} /><Stat label="Best Efficiency" value={perfWatt(efficient).toFixed(2)} /><Stat label="Max Scalability" value={maxScalability.maxScalability} /></div></Panel>
