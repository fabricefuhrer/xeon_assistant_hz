...
function AlternativeCard({ title, cpu, color, note, top }: { title: string; cpu: Cpu | null; color: string; note: string; top: Cpu | null }) {
  if (!cpu) {
    return <div style={{ border: `1px solid ${color}`, borderRadius: 7, padding: 12, background: "rgba(255,255,255,.03)", minHeight: 118 }}><div style={{ color, fontWeight: 800, marginBottom: 8 }}>{title}</div><div style={{ color: "#9fb0bd", fontSize: 12 }}>No compatible option in the selected CPU pool.</div></div>;
  }
  ...
  return <div style={{ border: `1px solid ${color}`, borderRadius: 7, padding: 12, background: "rgba(255,255,255,.03)", minHeight: 118 }}>
    <div style={{ color, fontWeight: 800, fontSize: 12, textTransform: "uppercase", marginBottom: 8 }}>{title}</div>
    <div style={{ fontWeight: 900, fontSize: 21, marginBottom: 4 }}>Xeon {cpu.sku}</div>
    <div style={{ color: "#c9d6e0", fontSize: 11, marginBottom: 9 }}>{note}</div>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 6, fontSize: 11 }}>
      ...
      <div><b>{cpu.maxScalability}</b><br /><span style={{ color: "#9fb0bd" }}>Socket</span></div>
    </div>
    {top && <div style={{ marginTop: 10, borderTop: "1px solid rgba(97,145,175,.25)", paddingTop: 8, display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 6, fontSize: 11 }}>
      <div><b>{perfDelta}%</b><br /><span style={{ color: "#9fb0bd" }}>Perf vs Rec.</span></div>
      <div><b>{priceDelta > 0 ? `+${priceDelta}%` : `${priceDelta}%`}</b><br /><span style={{ color: "#9fb0bd" }}>Price</span></div>
      <div><b>{powerDelta > 0 ? `+${powerDelta}%` : `${powerDelta}%`}</b><br /><span style={{ color: "#9fb0bd" }}>Power</span></div>
    </div>}
  </div>;
}
