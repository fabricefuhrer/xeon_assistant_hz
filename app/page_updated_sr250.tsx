...
function systemCompatible(cpu: Cpu, filters: Filters) {
  // A concrete system must be selected for compatibility validation.
  if (filters.system === "any") {
    return false;
  }

  // ST50 V3 and SR250 V3 are entry-level systems only.
  // Only Pentium G7400/G7400T, Xeon E-2400, and Xeon 6300 can appear here.
  if (filters.system === "st50-v3" || filters.system === "sr250-v3") {
    return isEntryServerCpu(cpu);
  }

  // Entry-level CPUs are locked to ST50 V3 and SR250 V3 only.
  if (isEntryServerCpu(cpu)) {
    return false;
  }

  // E-Cores are locked to SR630 V4 only.
  if (cpu.coreType === "E-Cores") {
    return filters.system === "sr630-v4";
  }

  // Xeon 6500 and Xeon 6700 P-Cores can run on SR rack systems.
  if (cpu.coreType === "P-Cores" && (cpu.family === "Xeon 6500" || cpu.family === "Xeon 6700")) {
    return isRackServer(filters.system);
  }
...
function allowedByFilters(cpu: Cpu, filters: Filters) {
  ...
  const rackSystems = ["sr630-v4", "sr650-v4", "sr680a-v4", "sr850-v4", "sr860-v4"];

  // Entry-level CPUs are locked to ST50 V3 and SR250 V3.
  if (isEntryServerCpu(cpu) && !["st50-v3", "sr250-v3"].includes(filters.system)) return false;

  // E-Cores are locked to SR630 V4 only.
  if (cpu.coreType === "E-Cores" && filters.system !== "sr630-v4") return false;

  // Xeon 6500 / 6700 P-Cores are locked to SR630 V4 and superior SR systems only.
  if (
    cpu.coreType === "P-Cores" &&
    ["Xeon 6500", "Xeon 6700"].includes(cpu.family) &&
    !rackSystems.includes(filters.system)
  ) return false;
